...
namespace FoxIDs.Controllers
{
    ...
    public class TOidcDownPartyController : TenantPartyApiController<Api.OidcDownParty, OidcDownParty>
    {
        ...
        public TOidcDownPartyController(TelemetryScopedLogger logger, IMapper mapper, ITenantRepository tenantService, ValidatePartyLogic validatePartyLogic) : base(logger, mapper, tenantService, validatePartyLogic)
        {
        ...
        public async Task<ActionResult<Api.OidcDownParty>> PostOidcDownParty([FromBody] Api.OidcDownParty party) => await Post(party, ap => Task.FromResult(true), async (ap, mp) => await validatePartyLogic.ValidateResourceScopesAsync(ModelState, mp));
        ...
        [ProducesResponseType(typeof(Api.OidcDownParty), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<ActionResult<Api.OidcDownParty>> PutOidcDownParty([FromBody] Api.OidcDownParty party) => await Put(party, ap => Task.FromResult(true), async (ap, mp) => await validatePartyLogic.ValidateResourceScopesAsync(ModelState, mp));

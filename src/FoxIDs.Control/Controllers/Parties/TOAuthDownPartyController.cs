...
namespace FoxIDs.Controllers
{
    ...
    public class TOAuthDownPartyController : GenericPartyApiController<Api.OAuthDownParty, OAuthDownParty>
    {
        ...
        public async Task<ActionResult<Api.OAuthDownParty>> PostOAuthDownParty([FromBody] Api.OAuthDownParty party) => await Post(party, ap => new ValueTask<bool>(true), async (ap, mp) => await validateOAuthOidcPartyLogic.ValidateModelAsync(ModelState, mp));
        ...
        [ProducesResponseType(typeof(Api.OAuthDownParty), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<ActionResult<Api.OAuthDownParty>> PutOAuthDownParty([FromBody] Api.OAuthDownParty party) => await Put(party, ap => new ValueTask<bool>(true), async (ap, mp) => await validateOAuthOidcPartyLogic.ValidateModelAsync(ModelState, mp));

...
namespace FoxIDs.Controllers
{
    ...
    public class TOAuthUpPartyController : GenericPartyApiController<Api.OAuthUpParty, Api.OAuthClaimTransform, OAuthUpParty>
    {
        ...
        public async Task<ActionResult<Api.OAuthUpParty>> PostOAuthUpParty([FromBody] Api.OAuthUpParty party) => await Post(party, ap => new ValueTask<bool>(validateApiModelOAuthOAuthPartyLogic.ValidateApiModel(ModelState, ap)), async (ap, mp) => await oidchDiscoveryReadUpLogic.PopulateModelAsync(ModelState, mp));
        ...
        [ProducesResponseType(typeof(Api.OAuthUpParty), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<ActionResult<Api.OAuthUpParty>> PutOAuthUpParty([FromBody] Api.OAuthUpParty party) => await Put(party, ap => new ValueTask<bool>(validateApiModelOAuthOAuthPartyLogic.ValidateApiModel(ModelState, ap)), async (ap, mp) => await oidchDiscoveryReadUpLogic.PopulateModelAsync(ModelState, mp));

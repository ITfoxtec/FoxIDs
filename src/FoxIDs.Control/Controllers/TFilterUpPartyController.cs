    ...
    public class TFilterUpPartyController : TenantApiController
    {
        ...
        public async Task<ActionResult<HashSet<Api.UpParty>>> GetFilterUpParty(string filterName)
        {
            try
            {
                ...
                var mUpPartys = filterName.IsNullOrWhiteSpace() ? await tenantService.GetListAsync<UpParty>(idKey, whereQuery: p => p.DataType.Equals(dataType)) : await tenantService.GetListAsync<UpParty>(idKey, whereQuery: p => p.DataType.Equals(dataType) && p.Name.Contains(filterName));
                var aUpPartys = new HashSet<Api.UpParty>(mUpPartys.Count());
                foreach(var mUpParty in mUpPartys)
                {

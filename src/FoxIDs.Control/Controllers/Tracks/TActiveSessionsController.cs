...
namespace FoxIDs.Controllers
{
    ...
    public class TActiveSessionsController : ApiController
    {
        ...
        public async Task<ActionResult<Api.PaginationResponse<Api.ActiveSession>>> GetActiveSessions(string filterUserIdentifier, string filterSub, string filterDownPartyName, string filterUpPartyName, string filterSessionId, string paginationToken = null)
        {
            ...
            (var mSessions, var nextPaginationToken) = await activeSessionLogic.ListSessionsAsync(filterUserIdentifier, filterSub, filterSessionId, filterDownPartyName, filterUpPartyName, paginationToken: paginationToken);
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<IActionResult> DeleteActiveSessions(string userIdentifier = null, string sub = null, string downPartyName = null, string upPartyName = null, string sessionId = null)
        {
            try
            {
                ...
                {
                    ModelState.TryAddModelError(string.Empty, $"Either the {nameof(userIdentifier)} or the {nameof(sub)} or the {nameof(downPartyName)} or the {nameof(upPartyName)} or the {nameof(sessionId)} parameter is required.");
            ...
            catch (FoxIDsDataException ex)
            {
                ...
                {
                    logger.Warning(ex, $"NotFound, Delete '{typeof(Api.ActiveSession).Name}' by user identifier '{userIdentifier}', SUB claim '{sub}', application '{downPartyName}', auth method '{upPartyName}', session ID '{sessionId}'.");
                    return NotFound(typeof(Api.ActiveSession).Name, new { userIdentifier, sub, downPartyName, upPartyName, sessionId }.ToJson());
                }
                throw;
            }
        }

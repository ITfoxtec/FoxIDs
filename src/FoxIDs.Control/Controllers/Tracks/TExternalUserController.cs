    ...
    public class TExternalUserController : ApiController
    {
        ...
        private async Task<ExternalUser> GetExternalUserAsync(Api.ExternalUserId userRequest)
        {
            ...
            if (mExternalUser == null)
            {
                ...
                (var mExternalUsers, _) = await tenantDataRepository.GetManyAsync<ExternalUser>(idKey, whereQuery: u => u.DataType.Equals(Constants.Models.DataType.ExternalUser) && u.UpPartyName.Equals(userRequest.UpPartyName) && u.RedemptionClaimValue.Equals(userRequest.RedemptionClaimValue));

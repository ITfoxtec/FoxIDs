...
namespace FoxIDs.Logic.Caches.Providers
{
    public class MongoDbCacheProvider : ICacheProvider
    {
        ...
        public async ValueTask DeleteAsync(string key)
        {
            ...
            _ = await collection.DeleteOneAsync(f => f.PartitionId.Equals(CachePartitionId, StringComparison.CurrentCulture) && f.Id.Equals(id, StringComparison.CurrentCulture));
        }
        ...
        public async ValueTask<bool> ExistsAsync(string key)
        {
            var id = GetId(key);
            var collection = mongoDbRepositoryClient.GetCacheCollection<CacheData>();
            var cachItem = await collection.Find(f => f.PartitionId.Equals(CachePartitionId, StringComparison.CurrentCulture) && f.Id.Equals(id, StringComparison.CurrentCulture)).FirstOrDefaultAsync();
        ...
        public async ValueTask SetAsync(string key, string value, int lifetime)
        {
            ...
            var collection = mongoDbRepositoryClient.GetCacheCollection<CacheData>();
            Expression<Func<CacheData, bool>> filter = f => f.PartitionId.Equals(cachItem.PartitionId, StringComparison.CurrentCulture) && f.Id.Equals(cachItem.Id, StringComparison.CurrentCulture);
        ...
        public async ValueTask<long> IncrementNumberAsync(string key, int? lifetime = null)
        {
            ...
            Expression<Func<CacheData, bool>> filter = f => f.PartitionId.Equals(CachePartitionId, StringComparison.CurrentCulture) && f.Id.Equals(id, StringComparison.CurrentCulture);

...
namespace FoxIDs.Logic
{
    public class SamlLogoutDownLogic : LogicBase
    {
        ...
        public SamlLogoutDownLogic(TelemetryScopedLogger logger, IServiceProvider serviceProvider, ITenantRepository tenantRepository, SequenceLogic sequenceLogic, Saml2ConfigurationLogic saml2ConfigurationLogic, IHttpContextAccessor httpContextAccessor) : base(httpContextAccessor)
        {
            this.logger = logger;
        ...
        private async Task<IActionResult> LogoutRequestAsync<T>(SamlDownParty party, Saml2Binding<T> binding)
        {
            ...
            try
            {
                ...
                switch (type)
                {
                    ...
                    case PartyType.Saml2:
                        return await serviceProvider.GetService<SamlLogoutUpLogic>().LogoutAsync(RouteBinding.ToUpParties.First().Id, GetSamlUpLogoutRequest(saml2LogoutRequest, party));
            ...
            {
                logger.Error(ex);
                return await LogoutResponseAsync(party.Id, samlConfig, saml2LogoutRequest.Id.Value, binding.RelayState, saml2LogoutRequest.Destination?.OriginalString, party.AuthnBinding.ResponseBinding.ToEnum<SamlBindingType>(), ex.Status);
            }
        }

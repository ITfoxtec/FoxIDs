...
namespace FoxIDs.Logic
{
    public class SamlLogoutUpLogic : LogicSequenceBase
    {
        ...
        private async Task<IActionResult> LogoutResponseAsync(SamlUpParty party, Saml2Http.HttpRequest samlHttpRequest)
        {
            ...
            var sequenceData = await sequenceLogic.GetSequenceDataAsync<SamlUpSequenceData>(partyName: party.Name, remove: party.DisableSingleLogout);
        ...
        {
            var sequenceData = await sequenceLogic.GetSequenceDataAsync<SamlUpSequenceData>(partyName: partyId.PartyIdToName(), remove: true);
        ...
        private async Task<IActionResult> SingleLogoutRequestAsync(SamlUpParty party, SamlUpSequenceData sequenceData)
        {
            ...
            await accessTokenSessionLogic.DeleteSessionAsync(session?.SessionIdClaim);
